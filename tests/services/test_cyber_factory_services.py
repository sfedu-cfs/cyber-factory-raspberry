...
def test_send_list_network_interface(obj):
    expected_result = [200, 201]
    ips = random_ip(count=2)
    result = obj.send_list_network_interfaces([{"name": "eth55", "ipAddress": ips[0]},
                                               {"name": "eth51", "ipAddress": ips[1]}])
    assert result in expected_result


def test_send_arp_record(obj):
    ip = random_ip()
    response = obj.send_arp_record({"ipAddress": ip, "macAddress": "00:00:00:00:00"})

    assert response["id"] is not None

    assert response["ipAddress"] == ip
    assert response["macAddress"] == "00:00:00:00:00"
