...
def test_port_collector_collect(mock_execute, mock_host_collect):
    ...
    a string containing the output of an Nmap scan.

    An instance of the PortCollector class is created. The collect method is then called.

    The expected_ports variable is created with the expected result as a ListPort object. It contains four SinglePort
