...
class BaseCountPacket(BaseModel):
    ...
    last_update: time = Field(default=dt.now(), description="The time of the last packet update.", exclude=True)
...
class EthernetLayer(BaseModel):
    ...
    arp_count: int = Field(0, description="The number of arp packets.")
...
class TransportLayer(BaseModel):
    ...
    tcp_count: int = Field(0, description="The number of tcp packets.")
    udp_count: int = Field(0, description="The number of udp packets.")
    icmp_count: int = Field(0, description="The number of icmp packets.")
    http_request: int = Field(0, description="The number of http request packets.")
    http_response: int = Field(0, description="The number of http response packets.")
